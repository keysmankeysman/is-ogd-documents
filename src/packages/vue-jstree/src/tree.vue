<template>
    <div :class="classes" role="tree" onselectstart="return false" class="tree">
    ...
    methods: {
      ...
      initializeDataItem (item) {
        function Model(item, textFieldName, valueFieldName) {
          ...
          this[valueFieldName] = item[valueFieldName] !== undefined ? item[valueFieldName] : item[textFieldName]
          this.icon = item.icon || ''
      ...
      onItemClick (oriNode, oriItem, isCtrlKeyDown) {
        if (this.multiple) {
          // если не нажат Ctrl выделяем также дочерние элементы
            if (!isCtrlKeyDown) {
        ...
        this.$emit('item-click', oriNode, oriItem, isCtrlKeyDown)
      },
